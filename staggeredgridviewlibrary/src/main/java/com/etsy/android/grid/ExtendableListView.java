    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                isAtedge = false;
                this.scrollTo(0, 0);
                if (onExtendableListViewScrollListener != null) {
                    onExtendableListViewScrollListener.up();
                }
                break;
    ...
    private boolean onTouchMove(final MotionEvent event) {
        ...
        switch (mTouchMode) {
            ...
            case TOUCH_MODE_DONE_WAITING:
                ...
                // scroll than a tap
                startScrollIfNeeded(event, y);
                break;
            case TOUCH_MODE_SCROLLING:
//            case TOUCH_MODE_OVERSCROLL:
                scrollIfNeeded(event, y);
                break;
        }
     ...
     */
    private boolean startScrollIfNeeded(final MotionEvent event, final int y) {
    ...
    public final static int PULLDOWN = 1;//下拉
    public final static int PULLUP = 2;//上拉
    public final static int PULLBOTH = 3;//全有
    public final static int PULLNULL = 0;//全没有
    public int PULLMODE = 0;

    public void setMode(int mode) {
        PULLMODE = mode;
    }

    private void scrollIfNeeded(final MotionEvent event, final int y) {
        ...
        if (mTouchMode == TOUCH_MODE_SCROLLING) {
            ...
            if (y != mLastY) {
                ...
                if (incrementalDeltaY != 0 && (PULLMODE == PULLNULL || !isAtedge)) {
                    atEdge = moveTheChildren(deltaY, incrementalDeltaY);
                }

                // Check to see if we have bumped into the scroll limit
                View motionView = this.getChildAt(motionIndex);
                if (!(PULLMODE == PULLNULL)) {
                    if (atEdge) {
                        isAtedge = true;
                        if (motionIndex >= 0 && (PULLMODE == PULLBOTH || PULLMODE == PULLDOWN)) {
                            if (onExtendableListViewScrollListener != null) {
                                onExtendableListViewScrollListener.sroll(PULLDOWN, (int) (downY - event.getY()));
                            }
                            this.scrollTo(0, (int) (downY - event.getY()));
                        } else if (motionIndex < 0 && (PULLMODE == PULLBOTH || PULLMODE == PULLUP)) {
                            if (onExtendableListViewScrollListener != null) {
                                onExtendableListViewScrollListener.sroll(PULLUP, (int) (downY - event.getY()));
                            }
                            this.scrollTo(0, (int) (downY - event.getY()));
                        }
                    } else {
    ...
    public interface OnExtendableListViewScrollListener {
        void sroll(int state, int y);

        void up();
    }

    private OnExtendableListViewScrollListener onExtendableListViewScrollListener;

    public void setOnExtendableListViewScrollListener(OnExtendableListViewScrollListener onExtendableListViewScrollListener) {
        this.onExtendableListViewScrollListener = onExtendableListViewScrollListener;
    }
